...
public class faceCarController : MonoBehaviour
{
    ...
    {        
        ...
        float sqrLenght = (zebraLine.position - this.transform.position).sqrMagnitude;//获取两个物体向量的平方长度
        //play animations
        animatorInfo = AVAnimator.GetCurrentAnimatorStateInfo(0);
        if (sqrLenght < 12 * 12 && sqrLenght > 6 * 6)
            {
            //slow down
            movement.z = 0.02f;
            nose.GetComponent<MeshRenderer>().material.color = Color.black;
        ...
        else if (sqrLenght < 7 * 7)
           { 
            sign.SetActive(eHMIType == 1 || eHMIType == 4);
            if ( RandomNum == 0)
                {
                    movement.z = 0.01f;
                    for (int i = 0; i<3; i++)
                        tips[i].GetComponent<Text>().text = "<color=red>Stop</color>"; 
                    stop.SetActive(eHMIType == 1 || eHMIType == 4);
                    if (eHMIType > 1)
                    {nose.GetComponent<MeshRenderer>().material.color = Color.red;
                    TriggerNegative();}
                }
            else
                {
                    ...
                    for (int i = 0; i<3; i++)
                        tips[i].GetComponent<Text>().text = "<color=green>Pass</color>";
                    if (eHMIType > 1)
                    {nose.GetComponent<MeshRenderer>().material.color = Color.green;
                    TriggerPositive();}
                }
            }
    }
